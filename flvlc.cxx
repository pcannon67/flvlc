...
void FLVLC::cb_length_changed(int64_t value)
{
  ...
  if ( nullptr != pl_window) { pl_window->select(playlist.position() ); }
}
...
void window_close_x(Fl_Widget*, void*)
{
  ...
  if ( nullptr != FLVLC::multimedia ) { delete FLVLC::multimedia;  }
  if ( nullptr != FLVLC::preferences ) { delete FLVLC::preferences; }
  exit(0);
}
...
{
  if  ( multimedia == NULL  or
		playlist.is_empty() or
...
void FLVLC::action_playlist()
{
  if ( playlist.count() != 0 and pl_window == nullptr )
  {
	...
	while ( pl_window->shown() ) { Fl::wait(); }

	const unsigned int sel_line = pl_window->selected();

	delete pl_window; pl_window = nullptr;
...
void FLVLC::action_subtitles()
{
  if (	FLVLC::state != MainWindow::STATE::STOP and
		playlist.count() != 0  and
		multimedia != nullptr and
		sbt_window == nullptr
	  )
  {
  	sbt_window = new SubtitlesWindow(Fl::event_x_root(),
									 Fl::event_y_root(),
									 multimedia
									 );

	while ( sbt_window->shown() ) { Fl::wait(); }

	delete sbt_window; sbt_window = nullptr;
  }
}
...
{
  if ( nullptr == pref_window and 
  	   nullptr != preferences
	 )
  {
  	pref_window = new Preferences_Window(Fl::event_x_root(),
									 Fl::event_y_root(),
									 preferences
									 );

	while ( pref_window->shown() ) { Fl::wait(); }

	delete pref_window; pref_window = nullptr;
  }
}
...
void FLVLC::action_play_pause()
{
   ...
   {
     state = (MainWindow::STATE::PAUSE == state) ? MainWindow::STATE::PLAY : MainWindow::STATE::PAUSE;
...
{
  url_window = new URL_Window( Fl::event_x_root(),
							   Fl::event_y_root()
							 );

  while ( url_window->shown() ) { Fl::wait(); }

  const char* addr = url_window->value();

  if ( (nullptr == addr) or (strlen(addr) == 0) ) 
  {
	delete url_window; url_window = nullptr;
  	return;
  }
  ...
  switch(event)
  {
  	case FL_KEYDOWN:
  	{
		...
		case FL_Right:
		  ...
		  {
			FLVLC::window.sl_video->value(FLVLC::window.sl_video->value() + FLVLC::video_percent_low);
			FLVLC::action_video();
		  }
		  break;
		...
		case FL_Up:
		  if ( FLVLC::state != MainWindow::STATE::STOP )
		  {
		  	FLVLC::window.sl_video->value(FLVLC::window.sl_video->value() + FLVLC::video_percent_high);
		  	FLVLC::action_video();
		  }
		  break;
  	...
  	case FL_MOUSEWHEEL:
	  ( 1 == Fl::event_dy() ) ? FLVLC::down_volume() : FLVLC::up_volume();
  	break;
  	...
  	case FL_PUSH:
  	  if ( Fl::event_button() == FL_LEFT_MOUSE and 0 != Fl::event_inside(FLVLC::window.display) ) 
  	  {
		FLVLC::action_play_pause();
	  }
...
void FLVLC::up_volume()
{
  ...
  if ( MainWindow::STATE::STOP == state ) { return; }

  int vol = FLVLC::window.sl_volume->value() + 15;
  if ( vol > 200 ) { vol = 200; } //ver initialize_volume()
...
void FLVLC::down_volume()
{
  ...
  int vol = FLVLC::window.sl_volume->value() - 15;
  if ( vol < 0 ) { vol = 0; } // ver initialize_volume()
  FLVLC::window.sl_volume->value(vol);
  FLVLC::action_volume();
}
...
void FLVLC::make_multimedia()
{
  ...
  FLVLC::multimedia = new Multimedia(preferences, FLVLC::window.get_id(), w, h);
...
void FLVLC::initialize_preferences()
{
  ...
  {
  	preferences = new Fl_Preferences(Fl_Preferences::Root::USER, "FLVLC", "flvlc");
...
{
  int int_value;
  if ( 0 == preferences->get("pls/save", int_value, 0) 
	  or int_value == 0 )
  {
	return; 
  }
