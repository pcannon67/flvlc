...
void SubtitlesWindow::init_choice(Fl_Choice* ch, Multimedia::Track track, ID id, const int current )
{
  ...
  { 
  	...
  	if ( id == ID::VIDEO ) { spn_vDelay->deactivate(); }
  
  	if ( id == ID::AUDIO ) { spn_aDelay->deactivate(); }
  }
}
...
void SubtitlesWindow::callback(Fl_Widget* w, void* data)
{
  ...
  else if ( win->ch_video == w )
  {
	...
	win->multimedia->set_video_subtitle(win->map_sub[ID::VIDEO][value]);
  }
  else if ( win->ch_audio == w )
  {
  	const int value = win->ch_audio->value();
	win->multimedia->set_audio_track(win->map_sub[ID::AUDIO][value]);
  }
  else if ( win->spn_vDelay == w )
  {
  	win->multimedia->set_video_subtitle_delay( win->spn_vDelay->value() );
  }
  else if ( win->spn_aDelay == w )
  {
  	win->multimedia->set_audio_track_delay( win->spn_aDelay->value() );
  }
}
